...
author: "David Repullo (DNI: 00000000-X), Chantal Zabala (DNI: 00000000-X), Rosalía (DNI: 00000000-X), ..."
format:
  revealjs:
    theme: [style.scss]
...
Nos quedamos con los datos del 2008 al 2019 de las elecciones. Después filtramos las encuestas por las mismas fechas, que no sean a pie de campo y que tenga un tamaño mayor que 500. Después nos quedamos con las encuestas que han estado mas de un dia "activas"
#| code-fold: true (para colpsar codigo)
...
surveys <- surveys |> 
  filter(between(date_elec, as_date("2008-01-01"), as_date("2019-01-01"))) |> 
  filter(exit_poll == FALSE) |> 
  filter(size > 500) |> 
  drop_na(size) |> 
  filter((field_date_to - field_date_from) > 1) |> 
  pivot_longer(cols = c("UCD" : "EV"), names_to = "siglas", values_to = "% votos") |> drop_na() 
